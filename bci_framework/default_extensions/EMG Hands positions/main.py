...
class FourClassMotorImagery(StimuliAPI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dashboard <= w.label('EMG Hand possitions', 'headline4')
        ...
        self.dashboard <= w.toggle_button(
            [
                (
                    'Start marker synchronization',
                    self.start_marker_synchronization,
                ),
                (
                    'Stop marker synchronization',
                    self.start_marker_synchronization,
                ),
            ],
            id='sync',
    ...
    def build_trials(self) -> None:
        ...
        self.trials = ['Paper', 'Scissors', 'Paper', 'Rock', 'Scissors', 'Rock'] * (w.get_value('repetitions')//2)
        # random.shuffle(self.trials)
        self.trials = [{'cue': trial} for trial in self.trials]

        self.pipeline_trial = [
            ['i_trial', 'intentional'],  # `intentional` is a slider reference
    ...
    # ----------------------------------------------------------------------
    def i_trial(
        self, cue: Literal['Paper', 'Rock', 'Scissors']
    ) -> None:
        """Cue visualization.
        ...
        self.cue_placeholder.class_name = (
            f'fa fa-{UNICODE_CUES[cue]} cue-intention'
        )
    ...
    def trial(
        ...
        self.cue_placeholder.class_name = f'fa fa-{UNICODE_CUES[cue]}'
        self.cue_placeholder.style = {'display': 'flex'}
