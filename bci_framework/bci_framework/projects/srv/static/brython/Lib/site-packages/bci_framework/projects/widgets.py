...
class Widgets:
    ...
    # ----------------------------------------------------------------------
    def title(self, text, mode='body1', style={}):
        """"""
        label = MDCComponent(html.SPAN(f'{text}'), style=style)
    ...
    def slider(self, label, min, max, valuenow, step=1, unit='', on_change=None, id=None, *args, **kwargs):
        ...
        if on_change:
            slider_.mdc.listen('MDCSlider:change', on_change)
    ...
    # # ----------------------------------------------------------------------
    # def combobox(self, label, options, valuenow, on_change=None):
        # """"""
        # form = MDCForm()
        # label_ = MDCComponent(html.SPAN(f'{label}'))
        # label_ .mdc.typography('subtitle1')
        # form <= label_
        # form <= MDCComponent(html.SPAN(f'{valuenow}', id='stimulus_rest')).mdc.typography('caption')
        # form.mdc.Select('', options=options)

        # if on_change:
            # pass
            # slider_.mdc.listen('MDCSlider:input', on_change)
