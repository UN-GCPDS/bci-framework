...
class P300GUI(StimuliAPI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.interface = json.loads(open(f'/root/interface.json?foo={time.time()}').read())
    ...
    def build_interface(self):
        ...
        for element in self.interface:
            ...
            if element['type'] == 'categorical':
                ...
                for option in element['options']:
                    categorical_area <= html.BUTTON(option, Class=f'cue show secondary primary-{key}')
                self.secondary_area <= categorical_area
                    ...
                    for increase in element['increase_modifiers']:                
                        increase_area <= html.BUTTON(increase, Class=f'cue show slider secondary primary-{key}')
                        
                    value_area <= html.BUTTON(element['value'], Class=f'cue secondary slider-value show primary-{key}')
                        
                    for decrease in element['decrease_modifiers']:                
                        decrease_area <= html.BUTTON(decrease, Class=f'cue show slider secondary primary-{key}')
            ...
            # elif element['type'] == 'boolean':
                ...
                # # boolean_area <= self.widgets.switch('', checked=True, on_change=None, id='')
        ...
        button_back = html.BUTTON(BACK_ARROW, Class=f'cue show secondary back-arrow')
        ...
        self.dashboard <= self.widgets.label('P300 GUI', typo='headline4')
        self.dashboard <= self.widgets.slider('Stimuli duration', min=50, max=300, step=10, value=100, id='stimuli_duration')
        self.dashboard <= self.widgets.slider('Stimuli interval', min=100, max=1000, step=10, value=350, id='stimuli_interval')
        
        self.dashboard <= self.widgets.button('Start stimlui', on_click=self.blink)
        self.show('primary')
    ...
    def blink(self):
        ...
        if self.frame == 'secondary':
            item = random.choice(self.secondary_area.select(f'.cue.secondary.show'))
        ...
        timer.set_timeout(self.blink, self.widgets.get_value('stimuli_interval'))
    
    # ----------------------------------------------------------------------
    def blink_element(self, item):
        """"""
        item.class_name += ' blink'
        timer.set_timeout(lambda :self.blink_element_off(item), self.widgets.get_value('stimuli_duration'))
