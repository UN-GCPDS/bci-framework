...
class Stream(EEGStream):
    ...
    def __init__(self):
        ...
        self.axis, self.time, self.lines = self.create_lines(mode='analog', time=-30, window=1000)
    ...
    def stream(self, data, topic, frame):
        ...
        if topic == 'eeg':
            ...
            self.timestamp_rises, diff = self.get_rises(aux, self.buffer_timestamp_resampled)
            self.lines[1].set_data(self.time, diff)
            
            self.feed()
            
        elif topic == 'marker' and self.timestamp_rises.size>0:
            recent = min([abs((datetime.fromtimestamp(t)-datetime.fromtimestamp(data.timestamp/1000)).total_seconds()) for t in self.timestamp_rises])
            logging.warning(recent)
