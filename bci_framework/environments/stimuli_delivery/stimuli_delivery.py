...
class StimuliDelivery:
    ...
    def update_experiments_list(self):
        """"""
        for i in range(self.parent.listWidget_projects_delivery.count()):
            item = self.parent.listWidget_projects_delivery.item(i)
    ...
    def load_experiment(self, experiment):
        """"""
        module = os.path.join('default_projects', experiment, experiment)
        self.preview_stream = LoadSubprocess(
            self.parent, f'{module}.py', debug=False, web_view='gridLayout_stimuli_webview', endpoint='delivery')

        self.parent.lineEdit_stimuli_ip.setText(
            f'{self.get_local_ip_address()}:5000')
